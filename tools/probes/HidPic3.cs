...
public static class HidPic3
{
    ...
    struct PHYSICAL_MONITOR { public IntPtr h; [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 128)] public string n; }
    ...
    static byte[] Build(ushort prop, ushort value)
    {
        ...
        byte[] msg;
        if (prop > 0xff)
            msg = new byte[] { (byte)(prop >> 8), (byte)prop, (byte)(value >> 8), (byte)value };
        else
            msg = new byte[] { (byte)prop, (byte)(value >> 8), (byte)value };
    ...
    static IntPtr phys; static PHYSICAL_MONITOR[] arr; static uint count;
    static bool InitVcp()
    {
        bool found = false;
        EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, (h, a, b, c) =>
        {
            ...
            count = 0; GetNumberOfPhysicalMonitorsFromHMONITOR(h, ref count);
            if (count == 0) return true;
            arr = new PHYSICAL_MONITOR[count];
            if (!GetPhysicalMonitorsFromHMONITOR(h, count, arr)) return true;
            phys = arr[0].h; found = true; return true;
        }, IntPtr.Zero);
        return found;
    }
    static string Get14()
    ...
    public static void Run()
    {
        ...
        try
        {
            ...
            try
            {
                // Test prop 0x2C (single-byte) values 0..7
                for (ushort v = 0; v <= 7; v++)
                {
                    bool wok = Send(hid, Build(0x2C, v));
                    Thread.Sleep(500);
                    Console.WriteLine("HID prop2C val=" + v + " write=" + wok + " " + Get14());
                }
            }
